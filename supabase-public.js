...
(() => {
  ...
  function quoteRequestFileKind(file){
  ...
  async function prepareQuoteRequestFile(file){
    if(!(file instanceof Blob))throw new Error('اختر مرفقًا صالحًا.');
    const kind=quoteRequestFileKind(file);
    if(!kind)throw new Error(`صيغة الملف ${file?.name||''} غير مدعومة.`);
    if(kind==='pdf'){
      if(file.size>8*1024*1024)throw new Error(`ملف PDF ${file?.name||''} أكبر من 8MB.`);
      return {blob:file,extension:'pdf',contentType:'application/pdf'};
    }
  ...
  window.flSubmitImageQuoteRequest=async function flSubmitImageQuoteRequest(payload={}){
    ...
    const inputFiles=Array.isArray(payload.files)?payload.files:(payload.file?[payload.file]:[]);
    const files=inputFiles.filter(Boolean).slice(0,10);
    ...
    if(!files.length)throw new Error('أضف صورة أو ملفًا واحدًا على الأقل.');
    if(inputFiles.length>10)throw new Error('الحد الأقصى 10 مرفقات لكل طلب.');
    ...
    const requestFolder=crypto.randomUUID?.() || `${Date.now()}-${Math.random().toString(36).slice(2)}`;
    const uploaded=[];
    const originalNames=[];
    try{
      for(let i=0;i<files.length;i++){
        const file=files[i];
        const prepared=await prepareQuoteRequestFile(file);
        const uuid=crypto.randomUUID?.() || `${Date.now()}-${i}-${Math.random().toString(36).slice(2)}`;
        const path=`incoming/${month}/${requestFolder}/${String(i+1).padStart(2,'0')}-${uuid}.${prepared.extension}`;
        const {error:uploadError}=await db.storage.from(quoteBucket).upload(path,prepared.blob,{contentType:prepared.contentType,upsert:false,cacheControl:'3600'});
        if(uploadError)throw new Error(`تعذر رفع المرفق ${i+1}: ${uploadError.message||uploadError}`);
        uploaded.push(path);
        originalNames.push(String(file?.name||`attachment-${i+1}`).slice(0,180));
      }
      const {data,error}=await db.from('quote_requests').insert({
        request_type:'image',customer_type,full_name,company_name,mobile,notes,
        image_path:uploaded[0],file_paths:uploaded,
        original_filename:originalNames[0]||'',original_filenames:originalNames,
        status:'new'
      }).select('id,request_code,created_at').single();
      if(error)throw new Error(`تم رفع المرفقات لكن تعذر تسجيل الطلب: ${error.message||error}`);
      saveCustomerProfile(customer_type==='company'?{full_name,company_name,mobile}:{full_name,mobile});
      return {...(data||{}),attachments_count:uploaded.length};
    }catch(error){
      if(uploaded.length){
        try{await db.storage.from(quoteBucket).remove(uploaded);}catch(_){}
      }
      throw error;
    }
  };
