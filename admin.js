// Flower Light Supabase admin controller — Stage 22.
...
(() => {
  ...
  const panelUnlockPrefix='fl_admin_panel_unlock_v2_';
  const panelUnlockKey=()=>`${panelUnlockPrefix}${adminPanel}_${String(currentAdminEmail||'unknown').toLowerCase()}`;
  ...
  const clearPanelUnlocks=()=>{try{
    const keys=[];
    for(let i=0;i<sessionStorage.length;i++){const key=sessionStorage.key(i);if(key?.startsWith(panelUnlockPrefix))keys.push(key);}
    keys.forEach(key=>sessionStorage.removeItem(key));
    sessionStorage.removeItem('fl_admin_panel_unlock_v1_1');sessionStorage.removeItem('fl_admin_panel_unlock_v1_2');
  }catch(_){}};
  ...
  async function renderLogin(initialMessage=''){
    ...
    document.getElementById('flLoginForm').addEventListener('submit',async e=>{
      ...
      try{
        ...
        msg.textContent=accessError?.message||String(accessError);
  ...
  function renderPermissions(){
    ...
    layout(`<div class="fl-cloud-head"><div><h2>صلاحيات admin=2</h2><p>اربط حساب Supabase منفصلًا للمدير المساعد، ثم حدد الأجزاء التي يستطيع إدارتها. الرئيسية تبقى ظاهرة دائمًا.</p></div></div>
      ...
      <form id="flPermissionsForm" class="fl-cloud-card fl-permissions-card">
        ...
        <div class="fl-cloud-actions"><button class="fl-cloud-btn primary" id="flPermissionsSave" type="submit">ربط الحساب وحفظ الصلاحيات</button><button class="fl-cloud-btn" id="flPermissionsDefault" type="button">تحديد الأقسام والمنتجات فقط</button></div>
      </form>`);
