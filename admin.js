...
(() => {
  ...
  if(adminSubtitle) adminSubtitle.textContent = isPrimaryAdmin ? 'جميع أدوات الموقع والصلاحيات' : 'الأدوات المسموح بها من المدير الأساسي';
  ...
  const defaultAdmin2Permissions = ['sections','products'];
  let admin2Permissions = new Set(defaultAdmin2Permissions);
  let permissionsStorageReady = true;
  const allowedAdminViews = () => new Set(isPrimaryAdmin
    ? ['overview',...validPermissionKeys,'permissions']
    : ['overview',...admin2Permissions]);
  ...
  async function loadAdmin2Permissions(){
    permissionsStorageReady=true;
    try{
      const {data,error}=await db.rpc('get_admin_panel_permissions',{p_panel:'2'});
      if(error)throw error;
      admin2Permissions=new Set(normalizePermissionList(data));
    }catch(error){
      permissionsStorageReady=false;
      admin2Permissions=new Set(defaultAdmin2Permissions);
      console.warn('[Admin permissions] load failed',error?.message||error);
    }
  }
  ...
  async function renderPanelPassword(){
    ...
    const panelName=isPrimaryAdmin?'المدير الأساسي':'المدير المساعد';
  ...
  function renderOverview(){
    ...
    const permissionsNote=!isPrimaryAdmin && !permissionsStorageReady
      ? '<div class="fl-cloud-note bad">تعذر تحميل الصلاحيات المحفوظة؛ تظهر مؤقتًا صلاحيتا الأقسام والمنتجات. شغّل ملف ADMIN_PERMISSIONS_SETUP.sql.</div>'
      : '';
    layout(`<div class="fl-cloud-head"><div><h2>${isPrimaryAdmin?'إدارة الموقع بالكامل':'لوحة المدير المساعد'}</h2><p>${isPrimaryAdmin?'جميع أجزاء الموقع متاحة لك، بما فيها الأقسام والمنتجات وتحديد صلاحيات المدير المساعد.':'تظهر هنا فقط الأجزاء التي سمح بها المدير الأساسي.'}</p></div></div>
      ${statsHtml}
      ${permissionsNote}
  ...
  function renderPermissions(){
    const rows=adminViewItems.map(([key,label])=>`<label class="fl-permission-row"><span class="fl-permission-copy"><strong>${esc(label)}</strong><small>السماح لـ admin=2 بفتح وإدارة هذا الجزء</small></span><input type="checkbox" name="admin2_permission" value="${key}" ${admin2Permissions.has(key)?'checked':''}><span class="fl-permission-check" aria-hidden="true">✓</span></label>`).join('');
    const storageNote=permissionsStorageReady
      ? '<div class="fl-cloud-note ok">الصلاحيات محفوظة مركزيًا، وستُطبّق على admin=2 من أي جهاز.</div>'
      : '<div class="fl-cloud-note bad">شغّل ملف <b>ADMIN_PERMISSIONS_SETUP.sql</b> في Supabase أولًا، ثم حدّث الصفحة.</div>';
    layout(`<div class="fl-cloud-head"><div><h2>صلاحيات admin=2</h2><p>ضع علامة صح أمام الأجزاء التي تريد إظهارها للمدير المساعد. الرئيسية تبقى ظاهرة دائمًا.</p></div></div>
      ${storageNote}
      <form id="flPermissionsForm" class="fl-cloud-card fl-permissions-card">
        <div class="fl-permission-list">${rows}</div>
        <div class="fl-cloud-field"><label for="flPermissionsPassword">كلمة مرور admin=1 للتأكيد</label><input id="flPermissionsPassword" type="password" autocomplete="current-password" required placeholder="أدخل كلمة مرور لوحة المدير الأساسي"></div>
        <div class="fl-cloud-actions"><button class="fl-cloud-btn primary" id="flPermissionsSave" type="submit" ${permissionsStorageReady?'':'disabled'}>حفظ صلاحيات admin=2</button><button class="fl-cloud-btn" id="flPermissionsDefault" type="button">تحديد الأقسام والمنتجات فقط</button></div>
      </form>`);
    document.getElementById('flPermissionsDefault')?.addEventListener('click',()=>{
      body.querySelectorAll('input[name="admin2_permission"]').forEach(input=>{input.checked=defaultAdmin2Permissions.includes(input.value);});
    });
    document.getElementById('flPermissionsForm')?.addEventListener('submit',async event=>{
      ...
      const password=document.getElementById('flPermissionsPassword').value;
      ...
      try{
        const {data,error}=await db.rpc('set_admin2_permissions',{p_admin1_password:password,p_permissions:selected});
        if(error)throw error;
        admin2Permissions=new Set(normalizePermissionList(data));
        permissionsStorageReady=true;
        renderPermissions();notify('تم حفظ صلاحيات admin=2');
      }catch(error){
        const message=String(error?.message||error||'');
        notify(message.includes('ADMIN_1_PASSWORD_INVALID')?'كلمة مرور admin=1 غير صحيحة':'تعذر حفظ الصلاحيات: '+message);
        save.disabled=false;save.textContent='حفظ صلاحيات admin=2';
      }
    });
  }
  ...
  (async()=>{
    ...
    try{await loadAdmin2Permissions();await refresh();renderApp();}catch(err){body.innerHTML=`<div class="fl-cloud-wrap"><div class="fl-cloud-note bad">تعذر قراءة قاعدة البيانات: ${esc(err.message||err)}. نفّذ ملفات إعداد Supabase المطلوبة أولًا ثم أعد فتح الصفحة.</div></div>`;}
  })();
})();
