...
(() => {
  ...
  function productSpecsFormHtml(prod){
    ...
    const fixed=PRODUCT_SPEC_FIELDS.map(field=>{
      ...
      return `<div class="fl-cloud-field"><label>${esc(field.label)}${field.unit?` <span class="fl-spec-unit">(${esc(field.unit)})</span>`:''}</label><input data-spec-key="${esc(field.key)}" data-spec-label="${esc(field.label)}" data-spec-unit="${esc(field.unit||'')}" value="${esc(spec?.value||'')}" placeholder="${esc(field.placeholder||'')}"></div>`;
    }).join('');
    const custom=specs.filter(spec=>!PRODUCT_SPEC_KEYS.has(spec.key)).slice(0,3);
    while(custom.length<3) custom.push(null);
    const customHtml=custom.map((spec,index)=>`<div class="fl-custom-spec-row"><div class="fl-cloud-field"><label>اسم المواصفة الإضافية ${index+1}</label><input data-custom-spec-label="${index}" value="${esc(spec?.label||'')}" placeholder="مثال: نوع الدرايفر"></div><div class="fl-cloud-field"><label>القيمة</label><input data-custom-spec-value="${index}" value="${esc(spec?.value||'')}" placeholder="مثال: بدون فليكر"></div></div>`).join('');
  ...
  function collectProductSpecifications(){
    ...
    document.querySelectorAll('[data-spec-key]').forEach(input=>{
      ...
      if(!value) return;
      specs.push({
        key:String(input.dataset.specKey||'').trim(),
    ...
    for(let index=0;index<3;index+=1){
      ...
      if(label && value) specs.push({key:`custom_${index+1}`,label,value,unit:''});
    }
