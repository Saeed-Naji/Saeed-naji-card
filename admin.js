...
(() => {
  ...
  async function loadPublicSiteCatalog(){
    ...
    try{
      ...
      // Backward-compatible fallback for Stage 41 single catalog until Stage 43 SQL is applied.
      try{
  ...
  window.FLOWER_LIGHT_PRODUCTS = { catalog: [], chandeliers: [], balfon: [], extraSections: [] };
  renderPublicProfile();
  if (db) { loadPublicProfile(); loadPublicSiteCatalog(); loadCloudProducts(); }
    ...
    document.getElementById('flAnalyticsReset')?.addEventListener('click',async()=>{
      ...
      try{
        ...
        notify(/DELETE requires a WHERE clause/i.test(resetMessage)
          ? 'يلزم تشغيل ملف ANALYTICS_RESET_FIX_STAGE67.sql في Supabase مرة واحدة، ثم إعادة المحاولة.'
          : (resetMessage||'تعذر إعادة تعيين الإحصائيات. شغّل ملف ANALYTICS_RESET_FIX_STAGE67.sql ثم حاول مجددًا.'));
