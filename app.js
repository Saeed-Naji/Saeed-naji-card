...
(() => {
  ...
  function drawCatalogTemplateBackground(ctx, canvas) {
    ...
    ctx.beginPath();
    ctx.moveTo(0, 192);
    ctx.bezierCurveTo(220, 188, 310, 228, 455, 222);
    ctx.bezierCurveTo(610, 216, 735, 176, 1000, 202);
    ctx.lineTo(1000, 232);
    ctx.lineTo(0, 232);
    ctx.closePath();
    ctx.fill();
  ...
  async function drawCatalogPageHeader(ctx, item) {
    ...
    if (logoAsset?.image) {
      try {
        ctx.drawImage(logoAsset.image, 42, 34, 88, 88);
      } finally {
    ...
    const englishBrandTop = parts[0] || 'FLOWER';
    const englishBrandBottom = parts.slice(1).join(' ') || 'LIGHT';
    ...
    ctx.fillText(englishBrandTop.toUpperCase(), 152, 88, 440);
    ctx.fillStyle = '#2b2b2b';
    ctx.font = '800 40px Arial, sans-serif';
    ctx.fillText(englishBrandBottom.toUpperCase(), 272, 132, 330);

    ctx.direction = 'rtl';
    ctx.textAlign = 'right';
    ctx.fillStyle = '#3b3b3b';
    ctx.font = '700 23px Tajawal, Arial, sans-serif';
    ctx.fillText('إضاءة أفضل لحياة أجمل', 310, 122, 260);
    ctx.fillStyle = '#666666';
    ctx.font = '500 18px Arial, sans-serif';
    ctx.direction = 'ltr';
    ctx.textAlign = 'left';
    ctx.fillText('Better Lighting for a Brighter Life', 58, 155, 340);
    ctx.fillStyle = '#ff9f0a';
    ctx.fillRect(56, 134, 28, 4);
    ...
    if (warranty) {
      ...
      ctx.fillStyle = '#ff9f0a';
      ctx.beginPath();
      ctx.moveTo(badgeX + 30, badgeY);
      ctx.lineTo(badgeX + 86, badgeY);
      ctx.lineTo(badgeX + 106, badgeY + 20);
      ctx.lineTo(badgeX + 106, badgeY + 74);
      ctx.lineTo(badgeX + 58, badgeY + 102);
      ctx.lineTo(badgeX + 10, badgeY + 74);
      ctx.lineTo(badgeX + 10, badgeY + 20);
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = '#ffffff';
      ...
      ctx.textBaseline = 'middle';
      ctx.direction = 'ltr';
      ctx.fillText(String(warranty), badgeX + 58, badgeY + 49);
      ctx.restore();
      ...
      ctx.fillText('YEARS', 850, 74, 130);
      ctx.fillText('WARRANTY', 850, 106, 140);
      ctx.direction = 'rtl';
      ctx.textAlign = 'right';
      ctx.fillStyle = '#333333';
      ctx.font = '800 24px Tajawal, Arial, sans-serif';
      const ar = warranty === 1 ? 'ضمان سنة' : warranty === 2 ? 'ضمان سنتين' : warranty === 3 ? 'ضمان 3 سنوات' : `ضمان ${warranty} سنوات`;
      ctx.fillText(ar, 955, 140, 180);
    }
  }
  ...
  function drawCatalogFooter(ctx) {
    ...
    fillRoundedRect(ctx, 34, 1308, 56, 56, 28, '#ff9f0a');
    ctx.fillStyle = '#ffffff';
    ctx.font = '800 30px Arial, sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('☎', 62, 1338);

    ctx.direction = 'rtl';
    ctx.textAlign = 'right';
    ctx.fillStyle = '#4f4f4f';
    ctx.font = '700 19px Tajawal, Arial, sans-serif';
    ctx.fillText('للاستفسار والطلبات', 204, 1332, 160);
    ctx.direction = 'ltr';
    ctx.textAlign = 'left';
    ctx.fillStyle = '#202020';
    ctx.font = '800 22px Arial, sans-serif';
    ctx.fillText(phone, 100, 1360, 180);
    ...
    fillRoundedRect(ctx, 314, 1308, 56, 56, 28, '#ff9f0a');
    ctx.fillStyle = '#ffffff';
    ctx.font = '800 28px Arial, sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('◉', 342, 1338);

    ctx.direction = 'rtl';
    ctx.textAlign = 'right';
    ctx.fillStyle = '#4f4f4f';
    ctx.font = '700 19px Tajawal, Arial, sans-serif';
    ctx.fillText('زوروا موقعنا الإلكتروني', 540, 1332, 155);
    ctx.direction = 'ltr';
    ctx.textAlign = 'left';
    ctx.fillStyle = '#202020';
    ctx.font = '500 19px Arial, sans-serif';
    ctx.fillText(website, 378, 1360, 350);

    ctx.direction = 'rtl';
    ctx.textAlign = 'right';
    ctx.fillStyle = '#676767';
    ctx.font = '700 18px Tajawal, Arial, sans-serif';
    ctx.fillText('إضاءة ... لمستقبل أكثر إشراقاً', 956, 1338, 220);
    ctx.direction = 'ltr';
    ctx.textAlign = 'right';
    ctx.fillStyle = '#7b7b7b';
    ctx.font = '500 18px Arial, sans-serif';
    ctx.fillText('Lighting ... for a Brighter Tomorrow', 955, 1363, 245);
  }
  ...
  function buildPdfSpecRows(item) {
    ...
    if (item?.caption) rows.unshift({ label: 'الوصف', value: String(item.caption).trim() });
  ...
  function drawPdfSpecsTable(ctx, rows, topY) {
    ...
    const width = 924;
    const headerH = 48;
    const rowH = rows.length > 10 ? 48 : 54;
    const bodyH = rowH * rows.length;

    fillRoundedRect(ctx, x, topY, width, headerH, 14, '#ff9f0a');
    ctx.fillStyle = '#ffffff';
    ctx.font = '800 32px Tajawal, Arial, sans-serif';
    ctx.direction = 'rtl';
    ctx.textAlign = 'right';
    ctx.fillText('المواصفات الفنية', x + width - 22, topY + 33, 300);
    ctx.direction = 'ltr';
    ctx.textAlign = 'left';
    ctx.font = '800 24px Arial, sans-serif';
    ctx.fillText('Technical Specifications', x + 18, topY + 31, 280);
    ...
    rows.forEach((row, index) => {
      ...
      ctx.fillStyle = index % 2 === 0 ? '#fafafa' : '#f2f2f0';
      ctx.fillRect(x, y, width, rowH);
      ctx.strokeStyle = '#d8d6d0';
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(x, y + rowH);
      ctx.lineTo(x + width, y + rowH);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(x + labelW, y);
      ctx.lineTo(x + labelW, y + rowH);
      ctx.stroke();
      ...
      ctx.font = '700 20px Tajawal, Arial, sans-serif';
      const labelY = y + rowH / 2 + 8;
      ctx.fillText(row.label, x + width - 16, labelY, labelW - 28);

      ctx.fillStyle = '#171717';
      ctx.font = '700 19px Tajawal, Arial, sans-serif';
      ctx.fillText(String(row.value), x + labelW - 16, labelY, valueW - 32);
    });
    return bodyH + headerH;
  }
  ...
  async function createBrandedProductPdfPage(item, { images = [], singleMode = false, currentIndex = 0, totalImages = 1 } = {}) {
    ...
    const rowH = rows.length > 10 ? 48 : 54;
    const tableHeight = 48 + rows.length * rowH;
    const footerY = 1278;
    const sectionGap = 24;
    let detailsBlockHeight = 128;
    if (item?.name || item?.caption || item?.model) detailsBlockHeight = item?.caption ? 152 : 128;
    const imageTop = 258;
    const imageBottom = Math.max(520, footerY - tableHeight - detailsBlockHeight - sectionGap * 2);
    const imageArea = { x: 40, y: imageTop, w: 920, h: Math.max(286, imageBottom - imageTop) };
    const productNameY = imageArea.y + imageArea.h + 44;
    ...
    ctx.textAlign = 'right';
    ctx.fillStyle = '#ff9f0a';
    ctx.font = '800 40px Tajawal, Arial, sans-serif';
    const nameY = wrapCanvasText(ctx, productName, 956, productNameY, 918, 42, 2);

    ctx.fillStyle = '#202020';
    ctx.font = '800 24px Tajawal, Arial, sans-serif';
    const codeText = item?.model ? `رقم المنتج / الكود: ${String(item.model)}` : '';
    if (codeText) ctx.fillText(codeText, 956, nameY + 10, 918);

    const descBaseY = codeText ? nameY + 46 : nameY + 18;
    if (item?.caption) {
      ctx.fillStyle = '#6a6a6a';
      ctx.font = '600 20px Tajawal, Arial, sans-serif';
      wrapCanvasText(ctx, item.caption, 956, descBaseY, 918, 28, 2);
    }

    const tableY = footerY - tableHeight - 24;
